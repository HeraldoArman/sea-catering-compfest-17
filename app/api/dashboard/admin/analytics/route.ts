...
export async function GET(request: NextRequest) {
  ...
  if (
    (startParam && (!startDate || isNaN(startDate.getTime()))) ||
    (endParam && (!endDate || isNaN(endDate.getTime())))
  ) {
  ...
  const [{ sum: revenueCurrent }] = await db
    ...
    .where(
      startDate && endDate
        ? inRange(subscription.createdAt, startDate, endDate)
        : sql`TRUE`,
    )
    .execute();
  const [{ sum: revenuePrevious }] = await db
    ...
    .where(
      prevStart && prevEnd
        ? inRange(subscription.createdAt, prevStart, prevEnd)
        : sql`TRUE`,
    )
    .execute();
  ...
  const [{ count: newUsers }] = await db
    ...
    .where(
      startDate && endDate
        ? inRange(user.createdAt, startDate, endDate)
        : sql`TRUE`,
    )
    .execute();
  const [{ count: prevNewUsers }] = await db
    ...
    .where(
      prevStart && prevEnd
        ? inRange(user.createdAt, prevStart, prevEnd)
        : sql`TRUE`,
    )
    .execute();
  const [{ count: activeUsers }] = await db
    ...
    .execute();
  const usersChange = prevNewUsers
    ? ((Number(newUsers) - Number(prevNewUsers)) / Number(prevNewUsers)) * 100
    : 0;
  ...
  const [{ count: prevTotalSubs }] = await db
    ...
    .execute();
  const subsGrowth = prevTotalSubs
    ? ((Number(totalSubs) - Number(prevTotalSubs)) / Number(prevTotalSubs)) *
      100
    : 0;
